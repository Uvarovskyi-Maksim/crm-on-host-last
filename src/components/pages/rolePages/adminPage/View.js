  ...
  return (
    <div>
      {filterBlock && <div className="sort_block">
        ...
        <div  className={ `filter_btn ${ activeMenuItem[1] === true ? 'active_el' : ''}`} onClick={sortDate}>Отсортировать по Дате</div>
        <div className={ `filter_btn ${ activeMenuItem[2] === true ? 'active_el' : ''}`} onClick={sortDatePhone}>Отсортировать по Дате созвона</div>
        <div className={ `filter_btn ${ activeMenuItem[3] === true ? 'active_el' : ''}`} onClick={sortByEmail}>Отсортировать по Email</div>
        <div className={ `filter_btn ${ activeMenuItem[4] === true ? 'active_el' : ''}`} onClick={sortByName}>Отсортировать по Имени</div>
